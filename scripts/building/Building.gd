...
# Nainštancuje Inside_Build a zavolá jeho setup(size_cells); ak chýba, použije fallback.
func _spawn_interior() -> void:
	...
	for y in size_cells.y:
		for x in size_cells.x:
			floors.set_cell(Vector2i(x, y), floor_src, Vector2i(0, 0), 0)

	# Perimeter walls
	for x in size_cells.x:
		walls.set_cell(Vector2i(x, 0),                wall_src, Vector2i(0, 0), 0)                     # top
		walls.set_cell(Vector2i(x, size_cells.y - 1), wall_src, Vector2i(0, 0), 0)                     # bottom
	for y in size_cells.y:
		walls.set_cell(Vector2i(0, y),                wall_src, Vector2i(0, 0), 0)                     # left
		walls.set_cell(Vector2i(size_cells.x - 1, y), wall_src, Vector2i(0, 0), 0) 
